...
//    }
    ...
    for (int i = 0 ; i < 1000; i ++) {
        [self.testArray addObject:[NSString stringWithFormat:@"2--%d",i]];
    }
    dispatch_async(queue1, ^{
        for (int i = 0 ; i < 1000; i ++) {
            [self.testArray addObject:[NSString stringWithFormat:@"1--%d",i]];
        }
    });
    dispatch_async(queue1, ^{
        for (int i = 0 ; i < 1000; i ++) {
            NSString *string = [NSString stringWithFormat:@"2--%d",i];
            [self.testArray removeObject:string];
        }
    });
}
